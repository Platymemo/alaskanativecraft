...
public class HarpoonEntity extends PersistentProjectileEntity {
	...
	public Packet<ClientPlayPacketListener> createSpawnPacket() {
		...
		packet.writeVarInt(Registries.ENTITY_TYPE.getRawId(this.getType()));
	...
	public void tick() {
		if (this.state == State.BOBBING) {
			...
			if (vec3d.y > 0.032D) {
				this.setPitch((float) (MathHelper.atan2(vec3d.y, vec3d.horizontalLength()) * MathHelper.DEGREES_PER_RADIAN));
	...
	protected void onEntityHit(@NotNull EntityHitResult entityHitResult) {
		...
		DamageSource damageSource = this.getDamageSources().create(AlaskaDamageTypes.HARPOON, this, (owner == null ? this : owner));
		this.dealtDamage = true;
		...
		float g = 1.0F;
		if (this.getWorld() instanceof ServerWorld && this.getWorld().isThundering() && EnchantmentHelper.hasChanneling(this.harpoonStack)) {
	...
	public void onPlayerCollision(PlayerEntity player) {
		...
		if (entity == null || entity.getUuid() == player.getUuid()) {
			if (this.state != State.FLYING && !this.getWorld().isClient && (this.state == State.BOBBING || this.isNoClip()) && this.shake <= 0) {
